...
<?php 
    ...
    {
        ?>
    <form action=<?= self::$config["base_url"]."index.php/categoriesProduits/add"?> method="post">
        <!--Bouton de retour -->
        <div class="row">
        ...
        <h2 class="mb-4 text-center mt-2">Ajouter une catégorie de produit</h2>
                ...
                <select class="form-select" name="idUnite" aria-label="Default select example">
                    <?php
                        foreach ($unitesMesure as $unite) {
                            echo '<option value="'.$unite->getIdUnite().'">'.$unite->getUniteNom().'</option>';
                        }
                    ?>
                </select>
            </div>
